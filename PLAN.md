...
> "그때 보낸 **그거** 뭐였지?"
> 디스코드 1:1 DM을 **제대로** 검색하는 유저 설치형 봇. DM 창에서 `/검색 youtube`를 입력하면 나와 상대가 주고받은 메시지 중 그 글자가 들어간 **모든** 메시지를 디스코드 UI 그대로 보여준다.
...
| 기록 범위 | **과거 전체** 기록. 이후 새 메시지도 자동 반영. |
...
**언제 도는가**
- `/연동` 직후: 모든 DM 전체 백필. 최근 대화가 많은 채널부터 처리한다(`last_message_id` 내림차순). 그래야 자주 찾는 대화가 먼저 검색 가능해진다.
- 주기(`SYNC_INTERVAL_MIN`, 기본 30분): 채널 목록을 한 번 받아서 `last_message_id != newest_synced_id`인 채널만 증분 동기화한다.
- `/검색` 직전: 현재 DM 채널 하나만 최대 2초 동안 증분 동기화를 시도한다. 시간이 넘으면 있는 데이터로 검색하고 백그라운드에서 이어간다.
- `/동기화`: 수동 증분 동기화. 채널이 목록에 새로 생겼으면 백필도 한다.
- 프로세스가 재시작되면 `backfill_done = 0`인 채널부터 이어서 한다.

**한계 (README에 명시):** 증분 동기화는 과거 메시지의 수정과 삭제를 반영하지 못한다. `/동기화 전체:true`로 해당 사용자의 전체 재수집을 제공한다.
...
| 이름(ko) | 옵션 | 동작 |
|---|---|---|
| `search`(검색) | `검색어`(필수, 1~100자), `보낸사람`(전체/나/상대), `종류`(전체/링크/유튜브/이미지/파일), `기간`(전체/7일/30일/1년), `상대`(DM 밖에서 쓸 때 대화 상대 선택, 자동완성) | 키워드 검색 |
| `ai-search`(ai검색) | `질문`(필수), `상대`(자동완성) | AI 의미 검색 (베타) |
| `link`(연동) | 없음 | 연동 안내 → 토큰 입력 모달 |
| `unlink`(연동해제) | 없음 | 확인 버튼 → 완전 삭제 |
